...
# These are the most authoritative sources in each region.
# Used both for claim detection AND as reference sources in verdicts.
...
# Official health authority feeds per country
RSS_FEEDS = {
    'GLOBAL': [
    ...
    'US': [
        ...
        {'url': 'https://www.nutrition.gov/news-events/whats-new/rss.xml', 'authority': 'USDA'},
    ],
    'GB': [
        {'url': 'https://www.nhs.uk/news/heart-disease/rss/', 'authority': 'NHS'},
        {'url': 'https://www.food.gov.uk/rss-feeds', 'authority': 'FSA'},
        {'url': 'https://www.nutrition.org.uk/press-office/press-releases/rss', 'authority': 'BNF'},
    ],
    'DE': [
        {'url': 'https://www.bfr.bund.de/SharedDocs/RSS/EN/bfr_rss_EN.xml', 'authority': 'BfR'},
        {'url': 'https://www.dge.de/rss/aktuelles.xml', 'authority': 'DGE'},
    ],
    'FR': [
        {'url': 'https://www.anses.fr/fr/rss.xml', 'authority': 'ANSES'},
        {'url': 'https://www.mangerbouger.fr/rss.xml', 'authority': 'MangerBouger'},
    ],
    'AU': [
            ...
            try:
                ...
                for entry in feed.entries[:10]:
                    ...
                    all_claims.append({
                        ...
                        'engagement': 80,  # Official sources weighted higher
                        'hours_ago': 6,
                        'is_official': True,
                        'fetched_at': datetime.now(timezone.utc).isoformat(),
                    })
                print(f"  RSS {feed_info['authority']} ({region}): {len(feed.entries)} entries")
            except Exception as e:
                print(f"Warning: RSS {feed_info['authority']} failed: {e}")
